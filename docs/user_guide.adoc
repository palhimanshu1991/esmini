...
On top of setting options from command line, esmini now supports setting up options from esminiLib and esminiRMLib as well. This new functionality is added to set and unset options during runtime. This is useful for example to enable or disable logging to console, or to change log level etc. In addition, it is now possible to presist option settings between multilple runs of scenarios. There are two variants of the API functions i.e. one to set option value with peristence and one without. The API functions are:
