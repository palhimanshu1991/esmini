...
[source,c++]
----
void SE_LogToConsole(bool mode, bool persistent);
----

The `persistent` flag controls the lifespan of the setting. If `true`, the setting will persist for the remainder of the application lifetime, or until changed again. If `false`, the setting will remain only for the current scenario session. In other words, persistent setting will remain over full `SE_Init()` / `SE_Close()` cycles, while non-persistent setting will be reset to default (console enabled) by each `SE_Init()`.

An additional way to control non-persistent setting of console logging is to set or unset the launch option `--disable_stdout` using the API functions:
...
- Full path can be provided, e.g. `"../myresult/scenario_2.txt"`.
...
The setting is persistent over scenario runs. Normally it should be called before SE_Init(). If called during the run of a scenario, the current log file will be closed and a new with the specified name and/or location will be created.
