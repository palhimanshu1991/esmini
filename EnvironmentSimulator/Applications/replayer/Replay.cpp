      ...
      create_datfile_(create_datfile)
{
    ...
    for (size_t i = 0; i < scenarios_.size(); i++)
    {
        ...
        if (file_.fail())
        {
            ERROR("Cannot open file: {}", scenarios_[i]);
    ...
    {
        ERROR_AND_QUIT("Too few scenarios loaded, use single replay feature instead\n");
    }
    ...
    for (size_t i = 0; i < scenarioData.size(); i++)
    {
        ...
        INFO("Scenarios corresponding to IDs ({}:{}): {}", i * 100, (i + 1) * 100 - 1, FileNameOf(scenario_tmp));
    }
...
void Replay::GetReplaysFromDirectory(const std::string dir, const std::string sce)
{
    ...
    {
        ERROR_AND_QUIT("No valid directory given, couldn't open {}", dir);
    }
    ...
    while ((file = readdir(directory)) != nullptr)
    {
        ...
        if (file->d_type == DT_DIR && filename.find(sce) != std::string::npos)
        {
            ...
            {
                ERROR("Couldn't open nested directory {}{}", dir, filename);
            }
    ...
    if (scenarios_.empty())
    {
        ERROR_AND_QUIT("Couldn't read any scenarios named {} in path {}", sce, dir);
    }
}
