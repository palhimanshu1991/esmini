...
void Geometry::EvaluateDS(double ds, double* x, double* y, double* h) const
{
    ...
    (void)y;
    (void)h;
    INFO("Geometry virtual Evaluate");
}

void Line::Print() const
{
    INFO("Line x: {:.2f}, y: {:.2f}, h: {:.2f} length: {:.2f}", GetX(), GetY(), GetHdg(), GetLength());
}
...
{
    INFO("Arc x: {:.2f}, y: {:.2f}, h: {:.2f} curvature: {:.2f} length: {:.2f}", GetX(), GetY(), GetHdg(), curvature_, GetLength());
}
...
{
    INFO("Spiral x: {:.2f}, y: {:.2f}, h: {:.2f} start curvature: {:.2f} end curvature: {:.2f} length: {:.2f} {}",
        GetX(),
        GetY(),
        GetHdg(),
        GetCurvStart(),
...
{
    INFO("Poly3 x: {:.2f}, y: {:.2f}, h: {:.2f} length: {:.2f} a: {:.2f} b: {:.2f} c: {:.2f} d: {:.2f}",
        GetX(),
        GetY(),
        GetHdg(),
        GetLength(),
...
{
    INFO("ParamPoly3 x: {:.2f}, y: {:.2f}, h: {:.2f} length: {:.2f} U: {:.8f}, {:.8f}, {:.8f}, {:.8f} V: {:.8f}, {:.8f}, {:.8f}, {:.8f}",
        GetX(),
        GetY(),
        GetHdg(),
        GetLength(),
...
{
    INFO("Elevation: s: {:.2f} A: {:.4f} B: {:.4f} C: {:.4f} D: {:.4f}", GetS(), poly3_.GetA(), poly3_.GetB(), poly3_.GetC(), poly3_.GetD());
}

void LaneLink::Print() const
{
    INFO("LaneLink type: {} id: {}", type_, id_);
}
...
Lane::Material* Lane::GetMaterialByIdx(int idx) const
{
    ...
    {
        INFO("Lane::GetMaterialByIdx() -> index {} exceeds list size {}", idx, lane_material_.size());
        return nullptr;
    }
...
{
    INFO("LaneWidth: sOffset: {:.2f}, a: {:.2f}, b: {:.2f}, c: {:.2f}, d: {:.2f}", s_offset_, poly3_.GetA(), poly3_.GetB(), poly3_.GetC(), poly3_.GetD());
}
...
RoadMarkColor LaneRoadMark::ParseColor(pugi::xml_node node)
{
    ...
    if (node.attribute("color") != 0 && strcmp(node.attribute("color").value(), ""))
    {
        ...
        else
        {
            ...
            ERROR("Unexpected and unsupported roadmark color {}", node.attribute("color").value());
        }
    }
...
{
    INFO("LaneOffset s {:.2f} a {:.4f} b {:.2f} c {:.2f} d {:.2f} length {:.2f}",
        s_,
        polynomial_.GetA(),
        polynomial_.GetB(),
        polynomial_.GetC(),
...
{
    INFO("Lane: {}, type: {}, level: {}", id_, type_, level_);
...
int Road::GetLaneInfoByS(double s, int start_lane_section_idx, int start_lane_id, LaneInfo& lane_info, int laneTypeMask) const
{
    ...
    {
        ERROR("Error idx {} > n_lane_sections {}", lane_info.lane_section_idx_, lane_section_.size());
    }
    else
    {
        ...
        if (s > lane_section->GetS() + lane_section->GetLength() - SMALL_NUMBER || s < lane_section->GetS())
        {
            ...
            if (start_lane_id != 0 && (lane == 0 || !(laneTypeMask & lane_section->GetLaneById(lane_info.lane_id_)->GetLaneType())))
            {
                ...
                if (lane == 0)
                {
                    WARN("GetLaneInfoByS: No valid connecting lane (rid: {} s: {:.2f} lane_id {}) - looking for closest valid lane",
                        GetId(),
                        s,
                        lane_info.lane_id_);
                }
                ...
                {
                    WARN("GetLaneInfoByS: No connecting lane (rid: {} s: {:.2f} lane_id {}) matching type mask 0x%X - looking for closest valid lane",
                        GetId(),
                        s,
                        lane_info.lane_id_);
                }
                ...
                if (new_lane_index < 0)
                {
                    WARN("Failed to find a closest snapping lane - fall back to reference lane (id 0)");
                    lane_info.lane_id_ = 0;
                    return -1;
                }

                lane_info.lane_id_ = lane_section->GetLaneByIdx(new_lane_index)->GetId();
                INFO("GetLaneInfoByS: Moved to %d", lane_info.lane_id_);
            }
        }
    }
...
int Road::GetConnectingLaneId(RoadLink* road_link, int fromLaneId, int connectingRoadId) const
{
    ...
    {
        ERROR("No connecting road or junction at rid {} link_type {}", GetId(), OpenDrive::LinkType2Str(road_link->GetType()).c_str());
        return 0;
    }
    ...
    if (lane == nullptr)
    {
        ERROR("Failed to get connecting lane {} {} {}", GetId(), fromLaneId, connectingRoadId);
        return 0;
    }
    ...
    if (road_link->GetElementType() == RoadLink::ELEMENT_TYPE_ROAD)
    {
        ...
        {
            ERROR("Wrong connectingRoadId {} (expected {})", road_link->GetElementId(), connectingRoadId);
            return 0;
        }
    ...
    else if (road_link->GetElementType() == RoadLink::ELEMENT_TYPE_JUNCTION)
    {
        ...
        if (junction == 0)
        {
            ERROR("Error: junction {} not existing", road_link->GetElementId());
            return 0;
        }
...
Lane::Material* Road::GetLaneMaterialByS(double s, int lane_id) const
{
    ...
    {
        WARN("GetLaneMaterialByS: No valid lane {} at road {} s {:.2f}\n", lane_id, GetId(), s);
    }
...
Geometry* Road::GetGeometry(int idx) const
{
    if (idx < 0 || idx + 1 > (int)geometry_.size())
    {
        ...
        {
            ERROR("Road::GetGeometry index {} out of range [0:{}]", idx, geometry_.size());
        }
        return nullptr;
    }
...
{
    INFO("LaneSection: {:.2f}, {} lanes:", s_, lane_.size());
...
int LaneSection::GetLaneIdByIdx(int idx) const
{
    ...
    {
        ERROR("LaneSection::GetLaneIdByIdx Error: index {}, only {} lanes", idx, lane_.size());
        return 0;
    }
    else
    {
...
int LaneSection::GetConnectingLaneId(int incoming_lane_id, LinkType link_type) const
{
    ...
    if (GetLaneById(id) == 0)
    {
        ERROR("Lane id {} not available in lane section!", id);
        return 0;
    }
...
RoadMarkInfo Lane::GetRoadMarkInfoByS(int track_id, int lane_id, double s) const
{
    ...
    if (road == 0)
    {
        ERROR("Position::Set Error: track {} not available", track_id);
        lsec_idx = -1;
    }
    else
    {
    ...
    if (lsec == 0)
    {
        ERROR("Position::Set Error: lane section {} not available", lsec_idx);
    }
    else
    {
    ...
    if (lane == 0)
    {
        ERROR("Position::Set Error: lane section {} not available", lane_id);
    }
...
RoadLink::RoadLink(LinkType type, pugi::xml_node node) : contact_point_type_(ContactPointType::CONTACT_POINT_UNDEFINED), element_id_(-1)
{
    ...
    if (element_type == "road")
    {
        ...
        {
            ERROR("Missing contact point type");
        }
        else
        {
            ERROR("Unsupported contact point type: {}", contact_point_type);
    ...
    {
        ERROR("Unsupported element type: %s", element_type.c_str());
...
{
    INFO("Road id: {} length: {:.2f}", id_, GetLength());
...
void Road::AddSignal(Signal* signal)
{
    ...
    DEBUG("Add signal[{}]: \"{}\" type {} subtype {} to road {}", signal_.size(), signal->GetName(),
...
std::string RMObject::Type2Str(RMObject::ObjectType type)
{
    ...
    {
        ERROR("Unsupported object type: {}", t);
    }
...
RMObject::ObjectType RMObject::Str2Type(std::string type)
{
    ...
    ERROR("Unsupported object type: {} - interpret as NONE", type);
...
bool Road::IsDirectlyConnected(Road* road, double* curvature, int fromLaneId) const
{
    ...
    if (IsSuccessor(road, &contact_point, fromLaneId) || IsPredecessor(road, &contact_point, fromLaneId))
    {
        ...
        {
            ERROR("Unexpected contact type {} between roads {} and {}", contact_point, GetId(), road->GetId());
        }
        return true;
    }
...
bool Road::GetZAndPitchByS(double s, double* z, double* z_prim, double* z_primPrim, double* pitch, int* index) const
{
    if (GetNumberOfElevations() > 0)
    {
        ...
        if (elevation == NULL)
        {
            ERROR("Elevation error NULL, nelev: {} elev_idx: {}", GetNumberOfElevations(), *index);
            return false;
        }
...
bool Road::UpdateZAndRollBySAndT(double s, double t, double* z, double* roadSuperElevationPrim, double* roll, int* index)
{
    if (GetNumberOfSuperElevations() > 0)
    {
        ...
        {
            ERROR("Superelevation error NULL, nelev: {} elev_idx: {}", GetNumberOfSuperElevations(), *index);
            return false;
        }
...
Junction* OpenDrive::GetJunctionByIdx(int idx) const
{
    ...
    {
        ERROR("GetJunctionByIdx error (idx {}, njunctions {})", idx, junction_.size());
        return 0;
    }
}
...
std::string ReadAttribute(pugi::xml_node node, std::string attribute_name, bool required)
{
    if (!strcmp(attribute_name.c_str(), ""))
    {
        if (required)
        {
            WARN("Warning: Required but empty attribute");
        }
        return "";
    }
    ...
    else
    {
        if (required)
        {
            WARN("Warning: missing required attribute: {} -> {}", node.name(), attribute_name);
        }
    }
...
bool OpenDrive::LoadOpenDriveFile(const char* filename, bool replace)
{
    ...
    if (node == NULL)
    {
        ERROR("Invalid OpenDRIVE file, can't find OpenDRIVE element");
        return false;
    }
    ...
    for (pugi::xml_node road_node : node.children("road"))
    {
        ...
        for (pugi::xml_node type_node = road_node.child("type"); type_node; type_node = type_node.next_sibling("type"))
        {
            ...
            {
                WARN("Missing road type - setting default (rural)");
                r_type->road_type_ = Road::RoadType::ROADTYPE_RURAL;
            }
            else
            {
                WARN("Unsupported road type: {} - assuming rural", type.c_str());
                r_type->road_type_ = Road::RoadType::ROADTYPE_RURAL;
            }
            ...
            if (speed != NULL)
            {
                ...
                {
                    WARN("Unsupported speed unit: {} - set UNDEFINED", unit.c_str());
        ...
        if (link != NULL)
        {
            ...
            if (r->GetJunction() != -1)
            {
                ...
                if (successor == NULL)
                {
                    WARN("Warning: connecting road {} in junction {} lacks successor", r->GetId(), r->GetJunction());
                }
                if (predecessor == NULL)
                {
                    WARN("Warning: connecting road {} in junction {} lacks predesessor", r->GetId(), r->GetJunction());
                }
            }
        }
        ...
        if (super_elevation_profile != NULL)
        {
                 ...
                 super_elevation                = super_elevation.next_sibling("superelevation"))
            {
                ...
                {
                    ERROR("SuperElevation: Major error");
                }
            }
        }
        ...
        if (lanes != NULL)
        {
            for (pugi::xml_node_iterator child = lanes.children().begin(); child != lanes.children().end(); child++)
            {
                ...
                else if (!strcmp(child->name(), "laneSection"))
                {
                    ...
                    if (s > r->GetLength())
                    {
                        INFO("Truncating lane section {} of road {} at s={:.2f} (road length)",
                    ...
                    for (pugi::xml_node_iterator child2 = child->children().begin(); child2 != child->children().end(); child2++)
                    {
                        ...
                        if (strcmp(child2->name(), "left") != 0 && strcmp(child2->name(), "right") !=0 && strcmp(child2->name(), "center") != 0)
                        {
                            ...
                            {
                                ERROR("Unexpected lane side: {}", child2->name());
                                continue;
                            }
                        }
                        ...
                        for (pugi::xml_node_iterator lane_node = child2->children().begin(); lane_node != child2->children().end(); lane_node++)
                        {
                            ...
                            {
                                ERROR("Unexpected element: %s, expected \"lane\"", lane_node->name());
                                continue;
                            }
                            ...
                            {
                                ERROR("unknown lane type: {} (road id={})", lane_type_str, r->GetId());
                            }
                            ...
                            for (pugi::xml_node roadMark = lane_node->child("roadMark"); roadMark; roadMark = roadMark.next_sibling("roadMark"))
                            {
                                ...
                                {
                                    ERROR("Lane road mark type error");
                                }
                                ...
                                {
                                    ERROR("unknown lane road mark type: {} (road id={})", roadMark.attribute("type").value(), r->GetId());
                                }
                                ...
                                if (roadMark.attribute("weight") != 0 && strcmp(roadMark.attribute("weight").value(), ""))
                                {
                                    ...
                                    {
                                        ERROR("unknown lane road mark weight: {} (road id={}) setting to standard",
                                ...
                                {
                                    WARN("Missing lane road mark color: {} (road id={}), set to standard (white)",
                                ...
                                if (!roadMark.attribute("laneChange").empty())
                                {
                                    ...
                                    {
                                        ERROR("Lane roadmark lanechange error");
                                    }
                                    else
                                    {
                                        ...
                                        {
                                            ERROR("unknown lane road mark lane change: {} (road id={})",
                                    ...
                                    if (sub_type != NULL)
                                    {
                                        ...
                                        for (pugi::xml_node line = sub_type.child("line"); line; line = line.next_sibling("line"))
                                        {
                                            ...
                                            if (line.attribute("rule") != 0 && strcmp(line.attribute("rule").value(), ""))
                                            {
                                                ...
                                                {
                                                    ERROR("unknown lane road mark type line rule: %s (road id=%d)",
                                ...
                                if (lane_roadMarkType == 0)
                                {
                                    ...
                                    {
                                        WARN("No road mark created for road {} lane {}. Type {} not supported. Either switch type or add a roadMark <type> element.",
                                            r->GetId(),
                                            lane_id,
                                            roadMark_type);
                                    }
                                    ...
                                    if (sub_explicit != NULL)
                                    {
                                        ...
                                        for (pugi::xml_node line = sub_explicit.child("line"); line; line = line.next_sibling("line"))
                                        {
                                            ...
                                            if (line.attribute("rule") != 0 && strcmp(line.attribute("rule").value(), ""))
                                            {
                                                ...
                                                {
                                                    ERROR("unknown lane road mark type line rule: {} (road id={})",
                                                        line.attribute("rule").value(),
                                                        r->GetId());
                                                }
                                            }
                    ...
                    for (int i = 0; i < lane_section->GetNumberOfLanes(); i++)
                    {
                        ...
                        {
                            WARN("Warning: expected laneId {} missing of roadId {}. Found laneIds {} and {}",
                                lastLaneId - 1,
                                r->GetId(),
                                lastLaneId,
                                lane->GetId());
                        ...
                        if (lane->GetLaneType() & roadmanager::Lane::LaneType::LANE_TYPE_ANY_ROAD)
                        {
                            ...
                            {
                                ERROR("Unexpected lane id {}", lane->GetId());
                            }
                        }
                    }
                ...
                {
                    ERROR("Unsupported lane type: {}", child->name());
                }
            }
        }
        ...
        if (signals != NULL)
        {
            ...
            for (pugi::xml_node signal = signals.child("signal"); signal; signal = signal.next_sibling())
            {
                if (!strcmp(signal.name(), "signal"))
                {
                    ...
                    {
                        ERROR("Signal dynamic check error");
                    }
                    ...
                    {
                        WARN("unknown dynamic signal identification: {} (road ids={})", signal.attribute("dynamic").value(), r->GetId());
                    }
                    ...
                    {
                        ERROR("Road signal orientation error");
                    }
                    ...
                    {
                        ERROR("unknown road signal orientation: {} (road ids={})", signal.attribute("orientation").value(), r->GetId());
                    }
                    ...
                    if (!type.empty())
                    {
                        ...
                        if (osi_type == static_cast<int>(Signal::OSIType::TYPE_UNKNOWN))
                        {
                            ...
                            if (osi_type == static_cast<int>(Signal::OSIType::TYPE_UNKNOWN))
                            {
                                WARN("Signal Type {} doesn't exists for country {}", type_to_find, country);
                            }
                        }
                    }
                ...
                {                    
                    ERROR_ONCE("INFO: signal element \"{}\" not supported yet", signal.name());
                }
            }
        }
        ...
        if (objects != NULL)
        {
            for (pugi::xml_node object = objects.child("object"); object; object = object.next_sibling("object"))
            {
                ...
                if (object.attribute("orientation") != 0 && strcmp(object.attribute("orientation").value(), ""))
                {
                    ...
                    {
                        WARN("unknown road object orientation: {} (road ids={})", object.attribute("orientation").value(), r->GetId());
                    }
                }
    ...
    for (pugi::xml_node junction_node = node.child("junction"); junction_node; junction_node = junction_node.next_sibling("junction"))
    {
        ...
        {
            WARN("Virtual junction type found. Not supported yet. Continue treating it as default type");
             ...
             connection_node                = connection_node.next_sibling("connection"))
        {
            if (connection_node != NULL)
            {
                ...
                {
                    WARN("Missing connecting road with id {}", connecting_road_id_str);
                    return false;
                }
                ...
                {
                    WARN("Warning: Connecting road (id {}) junction attribute ({}) is not referring back to junction {} which is making use of it",
                ...
                {
                    ERROR("Unsupported contact point: {}", contact_point_str);
                }
    ...
    if (!SetRoadOSI())
    {
        ERROR("Failed to create OSI points for OpenDrive road!");
    }
...
{
    INFO("Connection: incoming {} connecting {}", incoming_road_->GetId(), connecting_road_->GetId());
    for (size_t i = 0; i < lane_link_.size(); i++)
    {
        lane_link_[i]->Print();
    }
            ...
            for (int j = 0; j < connection->GetNumberOfLaneLinks(); j++)
            {
                ...
                if (laneId == lane_link->from_)
                {
                    if (counter == idx)
                    {
                        ...
                        if (lane == nullptr)
                        {
                            ERROR("Error: OpenDrive::GetJunctionConnection target lane not found! from {}, {} to {}, {}",
                                roadId,
                                laneId,
                        ...
                        {
                            WARN("OpenDrive::GetJunctionConnection target lane not driving! from %d, %d to %d, %d",
                                roadId,
                                laneId,
                                connection->GetConnectingRoad()->GetId(),
                                lane_link->to_);
...
bool Junction::IsOsiIntersection() const
{
    ...
    {        
        ERROR_ONCE("Type of roads are missing, cannot determine for OSI intersection or not, assuming that it is an intersection.");
        return true;
    }
}
...
Road* Junction::GetRoadAtOtherEndOfConnectingRoad(Road* connecting_road, Road* incoming_road) const
{
    ...
    {
        WARN("Unexpected: Road {} not a connecting road", connecting_road->GetId());
        return 0;
    }
    ...
    ERROR("Failed to find road at other end of the connecting road {} from road {}", connecting_road->GetId(), incoming_road->GetId());
    return nullptr;
}
...
int RoadPath::Calculate(double& dist, bool bothDirections, double maxDist)
{
    ...
    {
        ERROR("Invalid startpos road ID: {}", startPos_->GetTrackId());
        return -2;
    }

    if (targetRoad == nullptr)
    {
        ERROR("Invalid targetpos road ID: {}", targetPos_->GetTrackId());
        return -2;
    }
    ...
    for (i = 0; i < (bothDirections ? 2 : 1); i++)
    {
        ...
        if (link)
        {
            ...
            {
                ERROR("Unexpected contact point type: {}", contact_point);
            }
    ...
    for (i = 0; i < 100 && !found && unvisited_.size() > 0 && tmpDist < maxDist; i++)
    {
        ...
        else if (link->GetElementType() == RoadLink::ElementType::ELEMENT_TYPE_JUNCTION)
        {
            ...
            if (junction == nullptr)
            {
                ERROR("Failed to lookup junction with id {}", link->GetElementId());
            }
            for (size_t j = 0; junction && j < junction->GetNoConnectionsFromRoadId(pivotRoad->GetId()); j++)
            {
                ...
                if (nextRoad == targetRoad)  // target road reached
                {
                    ...
                    if (pivotRoad->IsSuccessor(nextRoad, &contact_point) || pivotRoad->IsPredecessor(nextRoad, &contact_point))
                    {
                        ...
                        {
                            ERROR("Unexpected contact point {}", OpenDrive::ContactPointType2Str(contact_point));
                            return -1;
                        }
                    }
                    else
                    {
                        ERROR("Failed to check link in junction");
                        return -1;
                    }
                    found = true;
                }
...
int OpenDrive::GetTrackIdxById(int id) const
{
    ...
    ERROR("OpenDrive::GetTrackIdxById Error: Road id {} not found", id);
    return -1;
}
...
int OpenDrive::GetTrackIdByIdx(int idx) const
{
    ...
    ERROR("OpenDrive::GetTrackIdByIdx: idx {} out of range [0:{}]", idx, road_.size());
    return 0;
}
    ...
    const
{
    ...
    for (int k = 0; k < 2; k++)
    {
        ...
        if (link->GetElementType() == RoadLink::ELEMENT_TYPE_ROAD)
        {
            if (link->GetElementId() == road2->GetId())
            {
                if (lane1_id != 0 && lane2_id != 0)
                {
                    ...
                    {
                        ERROR("Error LinkType {} not suppoered", link_type[k]);
                        return false;
                    }
                    if (lane_section == 0)
                    {
                        ERROR("Error lane section == 0");
                        return false;
                    }
        ...
        {
            ERROR("Error: LinkElementType {} unsupported", link->GetElementType());
        }
    }
...
int OpenDrive::CheckConnectedRoad(Road* road, RoadLink* link, ContactPointType expected_contact_point_type, RoadLink* link2)
{
    ...
    if (link2->GetElementType() == RoadLink::ElementType::ELEMENT_TYPE_ROAD)
    {
        if (link->GetElementId() == road->GetId())
        {
            ...
            {
                ERROR("Found connecting road from other end, but contact point is wrong (expected START, got {})",
...
int OpenDrive::CheckJunctionConnection(Junction* junction, Connection* connection)
{
    ...
    if (road == 0)
    {
        ERROR("Error no connecting road");
        return -1;
    }
    ...
    for (int i = 0; i < 2; i++)
    {
        if (link[i] != 0)
        {
            if (junction->GetType() == Junction::JunctionType::DIRECT)
            {
                if ((i == 0 &&  // link points to predecessor
                    ...
                    (i == 1 &&  // link points to successor
                     ...
                     link[i]->GetContactPointType() == ContactPointType::CONTACT_POINT_JUNCTION && link[i]->GetElementId() != junction->GetId()))
                {
                    ERROR("Expected direct junction linkedRoad to connect back to junction id {}, found id {}", link[i]->GetElementId());
                    return -1;
                }
                ...
                {
                    ERROR("Failed to find out contactpoint of direct junction incoming road");
                    return -1;
                }
            ...
            else
            {
                ...
                {
                    ERROR("Expected element type ROAD, found {}", ElementType2Str(link[i]->GetElementType()).c_str());
                    return -1;
                }
                ...
                if (link[i]->GetElementId() != connection->GetIncomingRoad()->GetId())
                {
                    ...
                    if (roadc == nullptr)
                    {
                        ERROR("Failed to resolve road {} in junction {}", link[i]->GetElementId(), junction->GetId());
                        return 0;
                    }
                    RoadLink* link2[2];
...
int OpenDrive::CheckLink(Road* road, RoadLink* link, ContactPointType expected_contact_point_type)
{
    ...
    if (link->GetElementType() == RoadLink::ElementType::ELEMENT_TYPE_ROAD)
    {
        ...
        if (connecting_road != 0)
        {
            ...
            {
                WARN("Warning: Reversed road link {}->{} not found. Might be a flaw in the OpenDRIVE description.",
                    road->GetId(),
                    connecting_road->GetId());
            }
        }
    ...
    else if (link->GetElementType() == RoadLink::ElementType::ELEMENT_TYPE_JUNCTION)
    {
        ...
        if (junction == nullptr)
        {
            ERROR("Info: Junction id {}, referred to by road {}, does not exist", link->GetElementId(), road->GetId());
            return -1;
        }
...
void OpenDrive::EstablishUniqueIds(pugi::xml_node& parent, std::string name, std::vector<std::pair<int, std::string>>& ids)
{
    ...
    for (auto node : parent.children(name.c_str()))
    {
        ...
        {
            id = max_id + 1;
            INFO("Assign internal id {} for {} {}", id, name, id_str);
        }
...
int OpenDrive::LookupRoadIdFromStr(std::string id_str)
{
    ...
    if (id == -1)
    {
        ERROR("Failed to lookup road id from string {}", id_str);
    }
...
int OpenDrive::LookupJunctionIdFromStr(std::string id_str)
{
    ...
    if (id == -1)
    {
        ERROR("Failed to lookup junction id from string {}", id_str);
    }
...
void OpenDrive::ParseGeoLocalization(const std::string& geoLocalization)
{
    ...
    for (const auto& attr : attributes)
    {
        ...
        {
            ERROR("Unsupported geo reference attr: {}", attr.first);
        }
    }

    if (std::isnan(geo_ref_.lat_0_) || std::isnan(geo_ref_.lon_0_))
    {
        WARN("cannot parse georeference: '{}'. Using default values.", geoLocalization);
        geo_ref_.lat_0_ = 0.0;
        geo_ref_.lon_0_ = 0.0;
    }
}
...
bool OpenDrive::LoadSignalsByCountry(const std::string& country)
{
    ...
    for (i = 0; i < file_name_candidates.size(); i++)
    {
        if (FileExists(file_name_candidates[i].c_str()))
        {
            ...
            if (fs.fail())
            {
                ERROR("Signal: Error to load traffic signals file - {}", file_name_candidates[i]);
    ...
    {
        ERROR("Failed to load {} file. Tried:", sign_filename);
        for (int j = 0; j < file_name_candidates.size(); j++)
        {
            INFO("  %s", file_name_candidates[j]);
        }
        return false;
    }
                    ...
                    for (int m = 0; m < number_of_roadmarks; m++)
                    {
                        ...
                        if (number_of_roadmarktypes != 0)
                        {
                                ...
                                lane_roadMark->GetType() == LaneRoadMark::RoadMarkType::SOLID_BROKEN)
                            {
                                ...
                                {
                                    ERROR_AND_QUIT("You need to specify at least 2 line for broken solid or solid broken roadmark type");
                                }
                            ...
                            for (int n = 0; n < number_of_roadmarklines; n++)
                            {
                                    ...
                                    if (lane_roadMark->GetType() == LaneRoadMark::RoadMarkType::BOTTS_DOTS)
                                    {
                                        ...
                                        while (true)
                                        {
                                            ...
                                            if (s_roadmarkline < SMALL_NUMBER || s_roadmarkline > s_end_roadmarkline - SMALL_NUMBER)
                                            {
                                                ...
                                                {
                                                    WARN("Roadmark length + space = 0 - ignoring");
                                                }
                                                break;
                                            }
                                        }
                                ...
                                {
                                    ERROR("LaneRoadMarkTypeLine {} for LaneRoadMarkType for LaneRoadMark {} for lane {} is not defined",
                                        n,
                                        m,
                                        lane->GetId());
                                }
...
int Position::GotoClosestDrivingLaneAtCurrentPosition()
{
    ...
    if (lane_section == 0)
    {
        ERROR("No lane section for idx {} - keeping current lane setting", lane_section_idx_);
        return -1;
    }
    ...
    if (lane_idx == -1)
    {
        ERROR("Failed to find a valid drivable lane");
        return -1;
    }
...
void Position::Track2Lane()
{
    ...
    if (road == 0)
    {
        ERROR("Position::Track2Lane Error: No road {}", track_idx_);
        return;
    }
    ...
    if (geometry == 0)
    {
        ERROR("Position::Track2Lane Error: No geometry {}", geometry_idx_);
        return;
    }
    ...
    if (lane_section == 0)
    {
        WARN("No lane section for idx {} - keeping current lane setting", lane_section_idx_);
        return;
    }
    ...
    if (lane_idx == -1)
    {
        ERROR("Failed find closest lane");
        return;
    }
...
Position::ReturnCode Position::XYZ2TrackPos(double x3, double y3, double z3, int mode, bool connectedOnly, int roadId, bool check_overlapping_roads)
{
    ...
    {
        ERROR("Unexpected: No closest OSI point found!");
    }
...
bool Position::EvaluateRoadZHPR()
{
    ...
    {
        ERROR("Failed to lookup road id {}", track_id_);
    }
...
Position::ReturnCode Position::Track2XYZ()
{
    ...
    if (road == 0)
    {
        ERROR("Position::Track2XYZ Error: No road {}", track_idx_);
        return ReturnCode::ERROR_GENERIC;
    }
    ...
    if (geometry == 0)
    {
        ERROR("Position::Track2XYZ Error: No geometry {}", geometry_idx_);
        return ReturnCode::ERROR_GENERIC;
    }
...
Position::ReturnCode Position::SetLongitudinalTrackPos(int track_id, double s)
{
    ...
    if ((road = GetOpenDrive()->GetRoadById(track_id)) == 0)
    {
        if (track_id >= 0)
        {
            ERROR("Position::Set Error: track {} not found", track_id);
        }
    ...
    if (s > road->GetLength())
    {
        ...
        {
            WARN("Position::Set Warning: s ({:.2f}) too large, track {} only {:.2f} m long", s, track_id_, road->GetLength());
        }
        s_ = road->GetLength();
...
Position::ReturnCode Position::MoveToConnectingRoad(RoadLink* road_link, ContactPointType& contact_point_type, double junctionSelectorAngle)
{
    ...
    if (road == 0)
    {
        ERROR("Invalid road id {}", road->GetId());
        return ReturnCode::ERROR_GENERIC;
    }

    if (road_link == 0)
    {
        ERROR("Lacking link of road id {}", road->GetId());
        return ReturnCode::ERROR_GENERIC;
    }

    if (road_link->GetElementId() == -1)
    {
        ERROR("No connecting road or junction at rid {} link_type {}", road->GetId(), OpenDrive::LinkType2Str(road_link->GetType()));
        return ReturnCode::ERROR_GENERIC;
    }
    ...
    if (lane_section == 0)
    {
        ERROR("No lane section rid {} ls_idx {} link_type  {}",
            road->GetId(),
            lane_section_idx_,
    ...
    if (lane == 0)
    {
        WARN("No lane rid {} lidx {} nlanes {} link_type {} lsecidx {}",
            road->GetId(),
            lane_idx_,
    ...
    if (road_link->GetElementType() == RoadLink::ELEMENT_TYPE_ROAD)
    {
        ...
        if (lane_link != 0)
        {
            ...
            if (new_lane_id == 0)
            {
                INFO("Road+ new lane id {}", new_lane_id);
            }
        }
        else
        {
            DEBUG("No lane link from rid {} lid {} to rid {}", GetTrackId(), GetLaneId(), road_link->GetElementId());
        }
        ...
        else
        {
            ...
            else if (junctionSelectorAngle >= 0.0)
            {
                ...
                for (int i = 0; i < n_connections; i++)
                {
                    ...
                    {
                        ERROR("Unexpected contact point type: {}", lane_road_lane_connection.contact_point_);
                    }
    ...
    if (new_lane_id == 0)
    {
        WARN("No connection from rid %d lid %d -> rid %d eltype %d - trying move to closest lane",
            road->GetId(),
            lane->GetId(),
        ...
        {
            ERROR("MoveToConnectingRoad: Unexpected contact point type: {}", (int)contact_point_type);
            return ReturnCode::ERROR_GENERIC;
        }

        INFO("{}onnection found (rid {} lid {})", ret_val < ReturnCode::OK ? "No c" : "C", GetTrackId(), GetLaneId());
    ...
    else if (road_link->GetContactPointType() == CONTACT_POINT_JUNCTION && road_link->GetElementType() == RoadLink::ELEMENT_TYPE_JUNCTION)
    {
        ...
        {
            ERROR("Unexpected contact point: {}", contact_point_type);
        }
    }
    else
    {
        ERROR("Unsupported contact point type {}", road_link->GetContactPointType());
        return ReturnCode::ERROR_GENERIC;
    }
                                          ...
                                          bool              updateRoute)
{
    ...
    for (int i = 0; done == false && i < max_links; i++)
    {
        ...
        {
            ERROR("MoveAlongS: Prevent move from road {} lane {} to opposite directed lane {}", track_id_, pos_save.lane_id_, lane_id_);
        ...
        else if (ret_val2 >= ReturnCode::OK)
        {
            ...
            else if (road->GetLaneWidthByS(GetS(), li.lane_id_) < SMALL_NUMBER)
            {
                ...
                else
                {
                    ...
                    {
                        ERROR("MoveAlongS Lane {} on road {} is or became zero width. Failed to move to a close lane with matching type 0x{}",
                            road->GetId(),
                            old_lane_id,
                            GetSnapLaneTypes());
                        ret_val = ReturnCode::ERROR_GENERIC;
                    ...
                    else
                    {
                        ...
                        INFO("MoveAlongS Lane {} on road {} is or became zero width, moved to closest available lane: {}",
                            old_lane_id,
                            road->GetId(),
                            GetLaneId());
                    }
...
Position::ReturnCode Position::SetLanePosMode(int track_id, int lane_id, double s, double offset, int mode, int lane_section_idx)
{
    ...
    if (lane_section != 0)
    {
        ...
        if (lane_idx_ == -1)
        {
            ERROR("lane_idx {} fail for lane id {}", lane_idx_, lane_id_);
            lane_idx_ = 0;
        }
    }
    else
    {
        ERROR("Position::Set (lanepos) Error - lanesection NULL lsidx {} rid {} lid {}", lane_section_idx_, road->GetId(), lane_id_);
    }
                              ...
                              int    lane_section_idx)
{
    ...
    {
        ERROR("roadmark_idx_ {} fail for lane id {}", roadmark_idx_, lane_idx_);
        roadmark_idx_ = 0;
    }
    ...
    if (lane_roadmarktype != 0)
    {
        ...
        {
            ERROR("roadmarktypeline_idx_ {} fail for roadmarktype_idx {}", roadmarkline_idx_, roadmarktype_idx_);
            roadmarkline_idx_ = 0;
        }
    }
    else
    {
        ERROR("roadmarktype_idx_ {} fail for roadmark_idx {}", roadmarktype_idx_, roadmark_idx_);
        roadmarkline_idx_ = 0;
    }
...
void Position::SetMode(PosModeType type, int mode)
{
    ...
    {
        ERROR("Unexpected Position SetMode type: {}", type);
        return;
    }
...
int Position::GetMode(PosModeType type)
{
    ...
    {
        ERROR("Unknown PosModeType: {} - returning 0", type);
    }
...
{
    INFO("	Track pos: (road_id {}, s {:.2f}, t {:.2f}, h {:.2f})", track_id_, s_, t_, h_);
}

void Position::PrintLanePos() const
{
    INFO("	Lane pos: (road_id {}, lane_id {}, s {:.2f}, offset {:.2f}, h {:.2f})", track_id_, lane_id_, s_, offset_, h_);
}

void Position::PrintInertialPos() const
{
    INFO("	Inertial pos: (x {:.2f}, y {:.2f}, z {:.2f}, h {:.2f}, p {:.2f}, r {:.2f})", x_, y_, z_, h_, p_, r_);
}

void Position::Print() const
{
    INFO("Pos({:.2f}, {:.2f}, {:.2f}) Rot({:.2f}, {:.2f}, {:.2f}) roadId {} laneId {} offset {:.2f} t {:.2f}",
        GetX(),
        GetY(),
        GetZ(),
        GetH(),
...
//#if 0  // Change to 1 to print some info on stdout - e.g. for debugging
        ...
        DEBUG("Dist {:.2f} Path (reversed): {} {}", dist, pos_b->GetTrackId(), roadIds);
		//printf("\n");
//#endif
    }
...
int Position::Distance(Position* pos_b, CoordinateSystem cs, RelativeDistanceType relDistType, double& dist, double maxDist) const
{
    ...
    else if (relDistType == RelativeDistanceType::REL_DIST_LATERAL || relDistType == RelativeDistanceType::REL_DIST_LONGITUDINAL)
    {
        ...
        {            
            WARN_ONCE("Lane coordinateSystem not supported yet. Falling back to Road coordinate system.");
    ...
    {
        ERROR("Unhandled case: cs {} reDistType {} freeSpace false", cs, relDistType);
        return -1;
    }
...
Position::ReturnCode Position::MoveRouteDS(double ds, double* remaining_dist, bool actualDistance)
{
    ...
    {
        ERROR("End of route at road_id {} lane_id {} s {:.2f}", GetTrackId(), GetLaneId(), GetS());
        SetRoute(nullptr);
    }
...
void PolyLineShape::CalculatePolyLine()
{
    ...
    for (size_t i = 0; i < vertex_.size(); i++)
    {
        ...
        {
            ERROR("Unexpected too few items in PolyLineShape vertex list");
            return;
        }
...
void ClothoidSplineShape::AddSegment(Position* posStart, double curvStart, double curvEnd, double length, double h_offset, double time)
{
    ...
    if (segments_.size() > 0)
    {
        if (std::isnan(curvStart))
        {
            ...
            WARN("Start curvature of ClothoidSpline segment {} not specified, use end curvature from previous segment({:.2f})",
                segments_.size(),
                curvStart);
        }
    }
    else
    {
        if (std::isnan(curvStart))
        {
            WARN("Start curvature of ClothoidSpline trajectory not specified, using default = 0.0");
            curvStart = 0.0;
        }
    }
    ...
    {
        WARN("End curvature of ClothoidSpline segment {} not specified, keep start curvature ({:.2f})", segments_.size(), curvStart);
        curvEnd = curvStart;
    }
...
int ClothoidSplineShape::Evaluate(double p, TrajectoryParamType ptype, TrajVertex& pos)
{
    ...
    if (segments_.size() < 1)
    {
        ERROR_AND_QUIT("You need to specify at least 1 segment in a ClothoidSpline shape");
    }
    ...
    if (ptype == TrajectoryParamType::TRAJ_PARAM_TYPE_TIME)
    {
        ...
        {
            WARN("ClothoidSplineShape::Evaluate Requested time {:.2f} outside range [{:.2f}, {:.2f}]", p, GetStartTime(), GetEndTime());
            p = GetLength();
        }
    }
    else if (p > GetLength())
...
void ClothoidSplineShape::Freeze(Position* ref_pos)
{
    if (spirals_.size() > 0)
    {
        INFO("Freezing clothoid spline trajectory. Clothoid shape already existing. Clearing it.");
        spirals_.clear();
    }
...
void ClothoidSplineShape::CalculatePolyLine()
{
    ...
    if (segments_.size() == 0)
    {
        WARN("Empty ClothoidSplineShape");
        return;
    }
...
void NurbsShape::CalculatePolyLine()
{
    ...
    {
        WARN("No valid heading in nurbs, set to zero");
        pline_.vertex_.back().h = 0.0;
    }
    if (std::isnan(pline_.vertex_.back().pitch))
    {
        WARN("No valid pitch in nurbs, set to zero");
...
int ClothoidShape::Evaluate(double p, TrajectoryParamType ptype, TrajVertex& pos)
{
    if (ptype == TrajectoryParamType::TRAJ_PARAM_TYPE_TIME)
    {
        ...
        {
            ERROR("Requested time {:.2f} outside range [{:.2f}, {:.2f}]", p, t_start_, t_end_);
            p = GetLength();
        }
    }
    else if (p > GetLength())
...
Position::ReturnCode Position::SetRouteS(double route_s)
{
    ...
    if (retval == ReturnCode::ERROR_END_OF_ROUTE)
    {
        INFO("Reached end of route, reset and continue");
        SetRoute(nullptr);
...
int Route::AddWaypoint(Position* position)
{
    ...
    if (minimal_waypoints_.size() > 0)
    {
        ...
        if (position->GetTrackId() == minimal_waypoints_.back().GetTrackId())
        {
            ...
            {
                // Ignore
                INFO("Ignoring additional waypoint for road {} (s {:.2f})", position->GetTrackId(), position->GetS());
            ...
            else  // at least two road-unique waypoints
            {
                ...
                INFO("Removing previous waypoint for same road {} (at s {:.2f})",
        ...
        if (retval == 0)
        {
            ...
            if (nodes.size() > 1)
            {
                ...
                for (int i = (int)nodes.size() - 1; i >= 1; i--)
                {
                    ...
                    INFO("Route::AddWaypoint Added intermediate waypoint {} roadId {} laneId {}",
    ...
    if (retval >= -1)
    {
        ...
        all_waypoints_.push_back(*position);
        INFO("Route::AddWaypoint Added waypoint {}: {}, {}, {:.2f}",
    ...
    {
        ERROR("Route::AddWaypoint Failed to add waypoint {}: {}, {}, {:.2f}",
            (int)minimal_waypoints_.size() - 1,
            position->GetTrackId(),
            position->GetLaneId(),
            position->GetS());
...
void Route::CheckValid()
{
    if (invalid_route_)
    {
        WARN("Warning: Route {} is not valid, will be ignored for the default controller.", getName());
...
Position::ReturnCode Route::SetTrackS(int trackId, double s)
{
    ...
    for (size_t i = 0; i < minimal_waypoints_.size(); i++)
    {
        ...
        {
            ERROR("Unexpected lack of connection in route at waypoint {}", i);
            return Position::ReturnCode::ERROR_GENERIC;
        }
        ...
        if (trackId == minimal_waypoints_[i].GetTrackId())
        {
            ...
            if (dist > GetLength() || dist < 0.0)
            {
                ...
                {
                    ERROR("Entity {} moved out of route", getObjName());
                    retval = Position::ReturnCode::ERROR_END_OF_ROUTE;
                }
            }
            else if (!OnRoute())
            {
                INFO("Entity {} on route", getObjName());
            }
    ...
    if (OnRoute())
    {
        INFO("Entity {} moved away from route", getObjName());
        waypoint_idx_ = -1;
    }
...
Position::ReturnCode Route::SetPathS(double s, double* remaining_dist)
{
    ...
    for (size_t i = 0; i < minimal_waypoints_.size(); i++)
    {
        ...
        else if (i == minimal_waypoints_.size() - 1)
        {
            ...
            ERROR("Entity {} moved passed route", getObjName());
            waypoint_idx_ = -1;
            return Position::ReturnCode::ERROR_END_OF_ROUTE;
        }
    }
...
Position* Route::GetWaypoint(int index)
{
    ...
    {
        ERROR("Waypoint index {} out of range ({})", index, minimal_waypoints_.size());
        return 0;
    }
...
Road* Route::GetRoadAtOtherEndOfConnectingRoad(Road* incoming_road) const
{
    ...
    if (junction == 0)
    {
        ERROR("Unexpected: Road {} not a connecting road", connecting_road->GetId());
        return 0;
    }
...
int Route::GetWayPointDirection(int index)
{
    ...
    if (road == nullptr)
    {
        ERROR("Waypoint {} invalid road id {}!", index, minimal_waypoints_[index].GetTrackId());
        return 0;
    }
    ...
    if (index < minimal_waypoints_.size() - 1)
    {
        ...
        {
            DEBUG("Relative heading not aligned with route direction");
            return -1 * direction;
        }
    }
    else if (index > 0)
    ...
    {
        WARN("Only one waypoint, no direction");
        return 0;
    }
}
