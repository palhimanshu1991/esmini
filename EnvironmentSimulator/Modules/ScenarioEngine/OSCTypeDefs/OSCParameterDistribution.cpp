...
int OSCParameterDistribution::Load(std::string filename)
{
    ...
    for (i = 0; i < file_name_candidates.size(); i++)
    {
        if (FileExists(file_name_candidates[i].c_str()))
        {
            ...
            if (!result)
            {
                INFO("{}: {} at offset (character position): {}", file_name_candidates[i], result.description(), result.offset);
                return -1;
            }
            else
            {
    ...
    {
        ERROR("Failed to load parameter distribution file {}. Tried:", filename);
        for (unsigned int j = 0; j < file_name_candidates.size(); j++)
        {
            INFO("   {}\n", file_name_candidates[j]);
        }
        WARN("continue without road description\n");
    }
    else
    {
        filename_ = file_name_candidates[i];
        INFO("Loaded {}", filename_);
    }
    ...
    else
    {
        ...
        {
            ERROR("Scenario filepath attribute missing");
        }
    }
    ...
    if (!deterministic.empty())
    {
        for (pugi::xml_node child_node = deterministic.first_child(); child_node; child_node = child_node.next_sibling())
        {
            ...
            if (child_node_name == "DeterministicMultiParameterDistribution")
            {
                ...
                {
                    ERROR("ValueSetDistribution missing");
                    return -1;
                }
            ...
            else if (child_node_name == "DeterministicSingleParameterDistribution")
            {
                ...
                {
                    ERROR("Missing single distribution parameter name");
                    return -1;
                }
                ...
                if (dist.empty())
                {
                    ERROR("Missing single distribution definition");
                    return -1;
                }
                ...
                else if (!strcmp(dist.name(), "DistributionRange"))
                {
                    ...
                    if (range.empty())
                    {
                        ERROR("Distribution range missing");
                        return -1;
                    }
                    ...
                    {
                        ERROR("Distribution range invalid range {:.2f}..{:.2f}", lower_limit, upper_limit);
                        return -1;
                    }
                ...
                {
                    ERROR("UserDefinedDistribution not yet supported");
                    return -1;
                }
                else
                {
                    ERROR("Unexpected distribution definition");
                    return -1;
                }
            }
            else
            {
                ERROR("Unexpected deterministic distribution type {}", child_node_name);
            }
        }
    }
    ...
    if (!stochastic.empty())
    {
        if (!deterministic.empty())
        {
            WARN("Found BOTH Deterministic and Stochastic distribution elements. Only one expected. Using Deterministic.");
        }
        else
        {
            WARN("Stochastic distributions not supported yet");
        }
    }
    else if (deterministic.empty())
    {
        ERROR("No distribution defined. Expected Determinstic or Stochastic");
    }
...
int OSCParameterDistribution::SetIndex(unsigned int index)
{
    ...
    {
        ERROR("Permutation index {} out of range (0..{})", index, GetNumPermutations() - 1);
        return -1;
    }
...
int OSCParameterDistribution::IncrementIndex()
{
    ...
    {
        ERROR("Can't increment index {}, would end up out of range (0..{})\n", index_, GetNumPermutations() - 1);
        return -1;
    }
...
int OSCParameterDistribution::SetRequestedIndex(unsigned int index)
{
    if (index >= GetNumPermutations())
    {
        ERROR("requested permutation index {} out of range (0..{})", index, GetNumPermutations() - 1);
        return 0;
    }
