...
int ReportTrafficCommand(osi3::TrafficCommand *tc, OSCPrivateAction *action, double time)
{
    ...
    switch (action->action_type_)
    {
        case OSCPrivateAction::ActionType::LAT_LANE_CHANGE:
        {
            ...
            LOG_INFO("Lane change action {} started for obj {}", action->GetName(), action->object_ ? action->object_->GetId() : -1);
        ...
        case OSCPrivateAction::ActionType::LONG_SPEED:
        {
            ...
            LOG_INFO("Speed action {} started for obj {}", action->GetName(), action->object_ ? action->object_->GetId() : -1);
        ...
        case OSCPrivateAction::ActionType::TELEPORT:
        {
            ...
            LOG_INFO("Teleport action {} started for obj {}", action->GetName(), action->object_ ? action->object_->GetId() : -1);
